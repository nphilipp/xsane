...
static const char * unit_string (SANE_Unit unit)
{
  double d;
...
static void button_new (GtkWidget * parent, const char *name, SANE_Word val,
...
static void scale_new (GtkWidget * parent, const char *name, gfloat val,
...
static void option_menu_new (GtkWidget *parent, const char *name, char *str_list[],
...
static void text_entry_new (GtkWidget * parent, const char *name, const char *val,
...
static void panel_build (GSGDialog * dialog)
{
  ...
  xsane_hbox = NULL;
  if (dialog->update_xsane_callback)
    xsane_hbox = (*dialog->update_xsane_callback) ();
  ...
  for (i = 1; i < dialog->num_elements; ++i)
	...
	snprintf (title, sizeof (title), "%s [%s]", opt->title, unit_string (opt->unit));
	...
	case SANE_TYPE_INT:
	  ...
	  switch (opt->constraint_type)
	      ...
	      for (j = 0; j < num_words; ++j)
		{
	      ...
	      str_list[j] = 0;
	      sprintf (str, "%d", val);
	      option_menu_new (parent, title, str_list, str, elem, dialog->tooltips, opt->desc);
	      free (str_list);
	      gtk_widget_show (parent->parent);
	      break;
	...
	case SANE_TYPE_STRING:
	  ...
	  switch (opt->constraint_type)
	    ...
	    case SANE_CONSTRAINT_NONE:
	      text_entry_new (parent, title, buf, elem, dialog->tooltips, opt->desc);
	      gtk_widget_show (parent->parent);
	      break;
	...
	case SANE_TYPE_BUTTON:
	  ...
	  gtk_signal_connect (GTK_OBJECT (button), "clicked", (GtkSignalFunc) push_button_callback, elem);
...
GSGDialog * gsg_create_dialog (GtkWidget *xsane_window,
		   ...
		   XSANECallback update_xsane_callback, void *update_xsane_arg,
		   int mode)
{
  ...
  if (status != SANE_STATUS_GOOD)
    {
      sprintf (buf, "Failed to open device `%s': %s.", device_name, sane_strstatus (status));
      gsg_error (buf);
      return 0;
    }
  ...
  dialog->xsane_window        = xsane_window;
  dialog->standard_window     = standard_window;
  dialog->advanced_window     = advanced_window;
  dialog->dev = dev;
  ...
  dialog->update_xsane_callback = update_xsane_callback;
  dialog->update_xsane_arg = update_xsane_arg;
